...
export const appRouter = router({
  ...
  admin: router({
    ...
    paymentTestLabRuns: adminProcedure.input(z.object({ status: z.enum(["running", "succeeded", "failed"]).optional(), search: z.string().max(64).optional(), page: z.number().int().min(1).optional(), pageSize: z.number().int().min(1).max(50).optional() }).optional()).query(({ input }) => getPaymentTestLabRuns(input ?? {})),
