...
import {
  calculateVedicSnapshot,
  __calculationInternals,
} from "./vedic-astrology-calculator";
...
describe("VedicAstrologyCalculator", () => {
  ...
  it("normalizes local time to UTC and produces a stable Lahiri contract", () => {
    ...
    expect(
      snapshot.planets.find(planet => planet.planet === "Moon")?.longitude
    ).toBeCloseTo(309.547025, 5);
  });
  ...
  it("creates D1 and D9 placements for all planets plus ascendant", () => {
    ...
    expect(
      snapshot.d1.every(
        placement => placement.sign >= 0 && placement.sign <= 11
      )
    ).toBe(true);
    expect(
      snapshot.d9.every(
    ...
    expect(
      snapshot.d9.find(placement => placement.planet === "Moon")?.sign
    ).toBe(8);
  });
  ...
  it("builds Vimshottari periods from the Moon nakshatra with a birth balance", () => {
    ...
    expect(new Date(periods[0].startUtc).toISOString()).toBe(
      snapshot.input.utcIso
    );
    expect(new Date(periods[0].endUtc).getTime()).toBeGreaterThan(
      new Date(periods[0].startUtc).getTime()
    );
  ...
  it("calculates documented Parashari full-sign graha aspects", () => {
    const aspects = __calculationInternals.makeGrahaAspects([
      ...
      { planet: "Moon", longitude: 91 },
      { planet: "Jupiter", longitude: 181 },
      { planet: "Saturn", longitude: 211 },
      { planet: "Ascendant", longitude: 301 },
      { planet: "Rahu", longitude: 241 },
      { planet: "Ketu", longitude: 61 },
    ]);
    ...
    expect(aspects.some(aspect => aspect.source === ("Rahu" as never))).toBe(
      false
    );
  });
  ...
  it("keeps the pure D9 and degree helpers deterministic", () => {
    ...
    expect(
      __calculationInternals.makeDivisionalPlacement("Sun", 0, 9)
    ).toMatchObject({ sign: 0, degreeInSign: 0 });
    expect(
      __calculationInternals.makeDivisionalPlacement("Sun", 30, 9)
    ).toMatchObject({ sign: 9, degreeInSign: 0 });
    expect(
      __calculationInternals.makeDivisionalPlacement("Sun", 60, 9)
    ).toMatchObject({ sign: 6, degreeInSign: 0 });
  });
  ...
  it.each([
    ...
    expect(() =>
      calculateVedicSnapshot({ ...moscowBirth, ...override })
    ).toThrow();
  });
});
