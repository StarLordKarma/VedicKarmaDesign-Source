...
import { ClientChangeHistory, InsertBookingRequest, InsertUser, bookingRequests, clientChangeHistory, paymentTestLabRuns, paymentTestLabRetentionSettings, paymentTestLabRetentionChanges, servicePackages, servicePricing, servicePricingCurrencies, servicePricingHistory, smokeTestRuns, users, receiptFiles, receiptRetentionSettings, receiptEmailAttempts, receiptEmailFailureAlerts, slaEmailAllowlist } from "../drizzle/schema";
...
export async function getPaymentTestLabRetentionSettings() { const db = await getDb(); if (!db) return { retentionDays: 90, lastCleanupAt: null, lastCleanupDeleted: null }; const [row] = await db.select().from(paymentTestLabRetentionSettings).where(eq(paymentTestLabRetentionSettings.id, 1)).limit(1); return row ?? { retentionDays: 90, lastCleanupAt: null, lastCleanupDeleted: null }; }
...
export async function cleanupExpiredPaymentTestLabRuns(retentionDays?: number) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const settings = await getPaymentTestLabRetentionSettings(); const days = retentionDays ?? settings.retentionDays; const cutoff = new Date(Date.now() - days * 86400000); const result = await db.delete(paymentTestLabRuns).where(lt(paymentTestLabRuns.startedAt, cutoff)); const deleted = Number(result[0]?.affectedRows ?? 0); await db.insert(paymentTestLabRetentionSettings).values({ id: 1, retentionDays: days, updatedBy: "system", lastCleanupAt: new Date(), lastCleanupDeleted: deleted }).onDuplicateKeyUpdate({ set: { lastCleanupAt: new Date(), lastCleanupDeleted: deleted } }); const recipient = process.env.OWNER_ALERT_EMAIL; const locale = process.env.OWNER_ALERT_LOCALE || "en"; const copy: Record<string,[string,string]>={en:["Payment Test Lab cleanup completed",`<p>Cleanup completed.</p><p>Deleted: <strong>${deleted}</strong> audit records.</p><p>Retention: ${days} days. No payloads or secrets are included.</p>`],ru:["Очистка Payment Test Lab завершена",`<p>Очистка завершена.</p><p>Удалено: <strong>${deleted}</strong> записей аудита.</p><p>Срок хранения: ${days} дней. Payload и секреты не включены.</p>`],de:["Payment Test Lab Bereinigung abgeschlossen",`<p>Bereinigung abgeschlossen.</p><p>Gelöscht: <strong>${deleted}</strong> Audit-Einträge.</p><p>Aufbewahrung: ${days} Tage. Keine Payloads oder Geheimnisse enthalten.</p>`],es:["Limpieza de Payment Test Lab completada",`<p>Limpieza completada.</p><p>Eliminados: <strong>${deleted}</strong> registros de auditoría.</p><p>Retención: ${days} días. No incluye payloads ni secretos.</p>`]}; const [subject,html]=copy[locale]??copy.en; if (recipient && ENV.resendApiKey && ENV.resendFromEmail) void fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${ENV.resendApiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: ENV.resendFromEmail, to: [recipient], subject, html }) }).catch(() => undefined); return { deleted, retentionDays: days, cutoff: cutoff.toISOString() }; }
