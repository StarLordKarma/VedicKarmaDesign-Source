...
import { ClientChangeHistory, InsertBookingRequest, InsertUser, bookingRequests, clientChangeHistory, paymentTestLabRuns, paymentTestLabRetentionSettings, servicePackages, servicePricing, servicePricingCurrencies, servicePricingHistory, smokeTestRuns, users, receiptFiles, receiptRetentionSettings, receiptEmailAttempts, receiptEmailFailureAlerts, slaEmailAllowlist } from "../drizzle/schema";
...
export async function updatePaymentTestLabRetentionDays(retentionDays: number, updatedBy: string) { if (!Number.isInteger(retentionDays) || retentionDays < 7 || retentionDays > 3650) throw new Error("Retention days must be between 7 and 3650."); const db = await getDb(); if (db) await db.insert(paymentTestLabRetentionSettings).values({ id: 1, retentionDays, updatedBy }).onDuplicateKeyUpdate({ set: { retentionDays, updatedBy } }); return retentionDays; }
export async function cleanupExpiredPaymentTestLabRuns(retentionDays?: number) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const settings = await getPaymentTestLabRetentionSettings(); const days = retentionDays ?? settings.retentionDays; const cutoff = new Date(Date.now() - days * 24 * 60 * 60 * 1000); const result = await db.delete(paymentTestLabRuns).where(lt(paymentTestLabRuns.startedAt, cutoff)); const deleted = Number(result[0]?.affectedRows ?? 0); await db.insert(paymentTestLabRetentionSettings).values({ id: 1, retentionDays: days, updatedBy: "system", lastCleanupAt: new Date(), lastCleanupDeleted: deleted }).onDuplicateKeyUpdate({ set: { lastCleanupAt: new Date(), lastCleanupDeleted: deleted } }); return { deleted, retentionDays: days, cutoff: cutoff.toISOString() }; }
