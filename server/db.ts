...
export async function getPaymentTestLabRetentionSettings() { const db = await getDb(); if (!db) return { retentionDays: 90, notificationLocale: "en", lastCleanupAt: null, lastCleanupDeleted: null }; const [row] = await db.select().from(paymentTestLabRetentionSettings).where(eq(paymentTestLabRetentionSettings.id, 1)).limit(1); return row ?? { retentionDays: 90, notificationLocale: "en", lastCleanupAt: null, lastCleanupDeleted: null }; }
export async function previewPaymentTestLabRetentionCleanup(retentionDays?: number) { const db = await getDb(); if (!db) return { eligibleCount: 0, retentionDays: retentionDays ?? 90 }; const settings = await getPaymentTestLabRetentionSettings(); const days = retentionDays ?? settings.retentionDays; const cutoff = new Date(Date.now() - days * 86400000); const [{ total }] = await db.select({ total: count() }).from(paymentTestLabRuns).where(lt(paymentTestLabRuns.startedAt, cutoff)); return { eligibleCount: total, retentionDays: days, cutoff: cutoff.toISOString() }; }
export async function getPaymentTestLabRetentionChanges() { const db = await getDb(); return db ? db.select().from(paymentTestLabRetentionChanges).orderBy(desc(paymentTestLabRetentionChanges.changedAt)).limit(50) : []; }
export async function updatePaymentTestLabRetentionDays(retentionDays: number, updatedBy: string) { if (!Number.isInteger(retentionDays) || retentionDays < 7 || retentionDays > 3650) throw new Error("Retention days must be between 7 and 3650."); const db = await getDb(); if (!db) return retentionDays; const before = await getPaymentTestLabRetentionSettings(); await db.insert(paymentTestLabRetentionSettings).values({ id: 1, retentionDays, updatedBy }).onDuplicateKeyUpdate({ set: { retentionDays, updatedBy } }); if (before.retentionDays !== retentionDays) await db.insert(paymentTestLabRetentionChanges).values({ oldRetentionDays: before.retentionDays, newRetentionDays: retentionDays, changedBy: updatedBy }); return retentionDays; }
export async function updatePaymentTestLabNotificationLocale(notificationLocale: "en" | "ru" | "de" | "es", updatedBy: string) { const db = await getDb(); if (!db) return notificationLocale; const settings = await getPaymentTestLabRetentionSettings(); await db.insert(paymentTestLabRetentionSettings).values({ id: 1, retentionDays: settings.retentionDays, notificationLocale, updatedBy }).onDuplicateKeyUpdate({ set: { notificationLocale, updatedBy } }); return notificationLocale; }
export async function getPaymentTestLabCleanupHistory(input: { triggeredBy?: string; from?: string; to?: string } = {}) { const db = await getDb(); if (!db) return []; const filters = [input.triggeredBy ? eq(paymentTestLabCleanupHistory.triggeredBy, input.triggeredBy) : undefined, input.from ? gte(paymentTestLabCleanupHistory.completedAt, new Date(`${input.from}T00:00:00.000Z`)) : undefined, input.to ? lt(paymentTestLabCleanupHistory.completedAt, new Date(`${input.to}T23:59:59.999Z`)) : undefined].filter(Boolean) as ReturnType<typeof eq>[]; return db.select().from(paymentTestLabCleanupHistory).where(filters.length ? and(...filters) : undefined).orderBy(desc(paymentTestLabCleanupHistory.completedAt)).limit(250); }
export function buildPaymentTestLabCleanupHistoryCsv(rows: Awaited<ReturnType<typeof getPaymentTestLabCleanupHistory>>) { return ["Triggered by,Retention days,Deleted records,Cutoff,Completed at", ...rows.map((row) => [row.triggeredBy,row.retentionDays,row.deletedCount,row.cutoff.toISOString(),row.completedAt.toISOString()].map((value) => `\"${String(value).replaceAll("\"", "\"\"")}\"`).join(","))].join("\n"); }
export async function cleanupExpiredPaymentTestLabRuns(input: { retentionDays?: number; triggeredBy?: string } = {}) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const settings = await getPaymentTestLabRetentionSettings(); const days = input.retentionDays ?? settings.retentionDays; const cutoff = new Date(Date.now() - days * 86400000); const result = await db.delete(paymentTestLabRuns).where(lt(paymentTestLabRuns.startedAt, cutoff)); const deleted = Number(result[0]?.affectedRows ?? 0); const completedAt = new Date(); await db.insert(paymentTestLabRetentionSettings).values({ id: 1, retentionDays: days, notificationLocale: settings.notificationLocale, updatedBy: "system", lastCleanupAt: completedAt, lastCleanupDeleted: deleted }).onDuplicateKeyUpdate({ set: { lastCleanupAt: completedAt, lastCleanupDeleted: deleted } }); await db.insert(paymentTestLabCleanupHistory).values({ triggeredBy: input.triggeredBy ?? "system", retentionDays: days, deletedCount: deleted, cutoff, completedAt }); const recipient = process.env.OWNER_ALERT_EMAIL; const copy: Record<string,[string,string]>={en:["Payment Test Lab cleanup completed",`<p>Cleanup completed.</p><p>Deleted: <strong>${deleted}</strong> audit records.</p><p>Retention: ${days} days. No payloads or secrets are included.</p>`],ru:["Очистка Payment Test Lab завершена",`<p>Очистка завершена.</p><p>Удалено: <strong>${deleted}</strong> записей аудита.</p><p>Срок хранения: ${days} дней. Payload и секреты не включены.</p>`],de:["Payment Test Lab Bereinigung abgeschlossen",`<p>Bereinigung abgeschlossen.</p><p>Gelöscht: <strong>${deleted}</strong> Audit-Einträge.</p><p>Aufbewahrung: ${days} Tage. Keine Payloads oder Geheimnisse enthalten.</p>`],es:["Limpieza de Payment Test Lab completada",`<p>Limpieza completada.</p><p>Eliminados: <strong>${deleted}</strong> registros de auditoría.</p><p>Retención: ${days} días. No incluye payloads ni secretos.</p>`]}; const [subject,html]=copy[settings.notificationLocale]??copy.en; if (recipient && ENV.resendApiKey && ENV.resendFromEmail) void fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${ENV.resendApiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: ENV.resendFromEmail, to: [recipient], subject, html }) }).catch(() => undefined); return { deleted, retentionDays: days, cutoff: cutoff.toISOString() }; }
export async function getPaymentTestLabRuns(input: { status?: "running" | "succeeded" | "failed"; search?: string; from?: string; to?: string; page?: number; pageSize?: number } = {}) { const db = await getDb(); const pageSize = Math.min(50, Math.max(1, input.pageSize ?? 10)); const page = Math.max(1, input.page ?? 1); if (!db) return { items: [], total: 0, page, pageSize, totalPages: 0 }; const clauses = [input.status ? eq(paymentTestLabRuns.status, input.status) : undefined, input.search ? like(paymentTestLabRuns.runId, `%${input.search.trim().slice(0, 64)}%`) : undefined, input.from ? gte(paymentTestLabRuns.startedAt, new Date(`${input.from}T00:00:00.000Z`)) : undefined, input.to ? lt(paymentTestLabRuns.startedAt, new Date(`${input.to}T23:59:59.999Z`)) : undefined].filter(Boolean) as ReturnType<typeof eq>[]; const where = clauses.length ? and(...clauses) : undefined; const [{ total }] = await db.select({ total: count() }).from(paymentTestLabRuns).where(where); const items = await db.select().from(paymentTestLabRuns).where(where).orderBy(desc(paymentTestLabRuns.startedAt)).limit(pageSize).offset((page - 1) * pageSize); return { items, total, page, pageSize, totalPages: Math.ceil(total / pageSize) }; }
