...
export async function cleanupExpiredPaymentTestLabRuns(retentionDays = 90) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const cutoff = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000); const result = await db.delete(paymentTestLabRuns).where(lt(paymentTestLabRuns.startedAt, cutoff)); return { deleted: Number(result[0]?.affectedRows ?? 0), retentionDays, cutoff: cutoff.toISOString() }; }
