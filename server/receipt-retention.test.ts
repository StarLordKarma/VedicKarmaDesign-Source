...
const mocks = vi.hoisted(() => ({ authenticateRequest: vi.fn(), cleanupExpiredReceiptFiles: vi.fn() }));
const { authenticateRequest, cleanupExpiredReceiptFiles } = mocks;

vi.mock("./_core/sdk", () => ({ sdk: { authenticateRequest: mocks.authenticateRequest } }));
vi.mock("./db", () => ({ cleanupExpiredReceiptFiles: mocks.cleanupExpiredReceiptFiles }));
...
describe("receipt retention scheduled handler", () => {
  ...
  it("cleans expired metadata only for an authenticated cron task", async () => {
    ...
    expect(response.json).toHaveBeenCalledWith({ ok: true, deleted: 4, retentionHours: 48 });
  });
