...
Scheduled work must be invoked by the host scheduler or a platform scheduler. Call `POST /api/scheduled/cleanup-receipts` and `POST /api/scheduled/evaluate-sla` from authenticated, private jobs; when `SCHEDULED_TASK_SECRET` is set, send it as a Bearer token. Do not run cron inside the container.
...
The GitHub connector is enabled for this project. The current authenticated account is `StarLordKarma`, and an existing private repository named `VedicKarmaDesign` was detected. No code is pushed automatically because selecting or overwriting a repository is a destructive ownership decision; confirm the target repository and whether the project should create a new private repository or be pushed to the existing one.
