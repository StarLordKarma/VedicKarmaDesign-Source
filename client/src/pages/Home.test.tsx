...
import { HTML_LOCALES, publicFormCopy, resolveLocale } from "./Home";
...
let packageData: Array<{ code: "basic" | "basic_plus" }> = [
  { code: "basic" },
  { code: "basic_plus" },
];
const renderHome = () =>
...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    pricing: {
      current: { useQuery: () => ({ data: pricingData }) },
      packages: { useQuery: () => ({ data: packageData }) },
      validatePromo: {
        useMutation: (options: typeof promoMutationOptions) => {
          promoMutationOptions = options;
          return promoMutationState;
        },
      },
      breakdownPdf: {
        useMutation: (options: typeof pdfMutationOptions) => {
          pdfMutationOptions = options;
          return pdfMutationState;
        },
      },
      emailBreakdownPdf: {
        useMutation: (options: typeof emailMutationOptions) => {
          emailMutationOptions = options;
          return emailMutationState;
        },
      },
    },
    booking: {
...
describe("Home booking payment UX", () => {
  ...
  it("disables an inactive package and selects the remaining active package", () => {
    ...
    expect(
      screen.getByRole("radio", { name: "Basic + numerology" })
    ).toBeChecked();
  });

  it("autosaves safe form values, restores a recent draft, and never restores consent", () => {
    renderHome();
    fireEvent.input(screen.getByLabelText("Preferred name"), {
      target: { value: "Maya" },
    });
    expect(
      JSON.parse(localStorage.getItem("vedic-booking-draft-v1") ?? "{}")
    ).toMatchObject({ name: "Maya", addon: false, currency: "USD" });
    expect(screen.getByTestId("draft-status")).toHaveTextContent(
      "Draft saved locally"
    );
    expect(screen.getByTestId("draft-relative-time")).toHaveTextContent(
      "Saved just now"
    );
  ...
  it("validates a promo code and updates the live total and included services", () => {
    ...
    expect(promoMutationState.mutate).toHaveBeenCalledWith({
      currency: "USD",
      addon: true,
      promoCode: "WELCOME10",
    });
    act(() =>
      promoMutationOptions.onSuccess?.({
        valid: true,
        code: "WELCOME10",
        discountPercent: 10,
        discountAmount: 3.5,
    ...
    expect(screen.getByTestId("applied-promo-status")).toHaveTextContent(
      "Promo code applied"
    );
  });
  ...
  it("shows a detailed checkout price breakdown and keeps the total mathematically consistent", () => {
    ...
    renderHome();
    expect(
      screen.getByRole("group", { name: "Price breakdown" })
    ).toHaveTextContent("Basic reading");
    expect(
      screen.getByRole("group", { name: "Price breakdown" })
  ...
  it("keeps price-breakdown tooltip copy aligned with the selected language", () => {
    renderHome();
    expect(
      screen.getByRole("button", { name: "Basic reading" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Indian numerology add-on/ })
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: "Базовое чтение" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Дополнение: индийская нумерология/ })
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Итого" })).toBeInTheDocument();
    fireEvent.change(screen.getAllByRole("combobox", { name: "Язык" })[0], {
      target: { value: "de" },
    });
    expect(
      screen.getByRole("button", { name: "Basisdeutung" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Zusatz: indische Numerologie/ })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Gesamtsumme" })
    ).toBeInTheDocument();
    fireEvent.change(screen.getAllByRole("combobox", { name: "Sprache" })[0], {
      target: { value: "es" },
    });
    expect(
      screen.getByRole("button", { name: "Lectura básica" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Complemento de numerología india/ })
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Total" })).toBeInTheDocument();
  });

  it.each([
    ["en", "Date of birth", "Format: MM/DD/YYYY", "name@example.com"],
    ["ru", "Дата рождения", "Формат: ДД.ММ.ГГГГ", "имя@пример.рф"],
    ["de", "Geburtsdatum", "Format: TT.MM.JJJJ", "name@beispiel.de"],
    ["es", "Fecha de nacimiento", "Formato: DD/MM/AAAA", "nombre@ejemplo.es"],
  ] as const)(
    "localizes browser language, examples, and field hints for %s",
    (selectedLocale, dateLabel, dateFormat, emailPlaceholder) => {
      window.history.replaceState({}, "", `/?lang=${selectedLocale}`);
      renderHome();
      expect(document.documentElement.lang).toBe(HTML_LOCALES[selectedLocale]);
      expect(
        screen.getByText(new RegExp(dateFormat.replace(/[./]/g, "\\$&")))
      ).toBeInTheDocument();
      expect(screen.getByLabelText(dateLabel)).toHaveAttribute(
        "lang",
        HTML_LOCALES[selectedLocale]
      );
      expect(screen.getByPlaceholderText(emailPlaceholder)).toBeInTheDocument();
    }
  );

  it.each(["en", "ru", "de", "es"] as const)(
    "shows application-controlled validation in %s",
    selectedLocale => {
      window.history.replaceState({}, "", `/?lang=${selectedLocale}`);
      renderHome();
      fireEvent.click(
        screen.getByRole("button", { name: COPY[selectedLocale].request })
      );
      expect(screen.getByRole("alert")).toHaveTextContent(
        publicFormCopy[selectedLocale].required
      );
    }
  );
  ...
  it("offers localized birth-field help, downloads the PDF, and shares its receipt link", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Exact time of birth/ })
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", { name: "Download price breakdown PDF" })
    );
    expect(pdfMutationState.mutate).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        labels: expect.objectContaining({
          ...
          basic: "Basic reading",
          total: "Total",
          disclaimer: expect.stringContaining("personal reflection"),
        }),
      })
    );
    act(() => {
      pdfMutationOptions.onSuccess?.({
        filename: "price.pdf",
        contentBase64: btoa("%PDF-1.7"),
        url: "/manus-storage/price-breakdowns/test.pdf",
      });
    });
    ...
    expect(screen.getByRole("link", { name: "WhatsApp" })).toHaveAttribute(
      ...
      expect.stringContaining(
        encodeURIComponent(
          "http://localhost:3000/manus-storage/price-breakdowns/test.pdf"
        )
      )
    );
    ...
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: "Share on device" }));
    });
    ...
    await act(async () => {
      fireEvent.click(
        screen.getByRole("button", { name: "Copy receipt link" })
      );
    });
    expect(
      screen.getByRole("button", { name: "Receipt link copied" })
    ).toBeInTheDocument();
  });
  ...
  it("persists the selected German language and resolves it on a later session", () => {
    ...
    expect(
      screen.getByText("Eine klarere Karte für Ihren inneren Himmel.")
    ).toBeInTheDocument();
    cleanup();
  ...
  it("restores the saved language and theme for the booking form", () => {
    ...
    renderHome();
    expect(
      screen.getByText("Un mapa más claro para tu cielo interior.")
    ).toBeInTheDocument();
    expect(
  ...
  it("highlights and shakes the consent block when submitting without acceptance", () => {
    ...
    fireEvent.change(screen.getByLabelText("Exact time of birth"), {
      target: { value: "14:30" },
    });
    fireEvent.change(screen.getByLabelText("City of birth"), {
      target: { value: "Delhi" },
    });
    fireEvent.change(screen.getByLabelText("Country of birth"), {
      target: { value: "India" },
    });

    fireEvent.click(
      screen.getByRole("button", { name: /Request my reading/i })
    );
    ...
    expect(screen.getByLabelText("Privacy consent")).toHaveAttribute(
      "aria-invalid",
      "true"
    );
    expect(
      screen.getByLabelText("Privacy consent").closest("label")
    ).toHaveClass("consent-error-shake");
  ...
  it("shows pending loading, success confirmation, and schedules the checkout redirect", async () => {
    ...
    const view = renderHome();
    expect(
      screen.getByRole("status", { name: "Booking progress" })
    ).toHaveTextContent("2 steps remaining");
    ...
    expect(
      screen.getByRole("button", { name: /Creating your secure checkout/i })
    ).toBeInTheDocument();
    ...
    expect(
      screen.getAllByText((_, node) =>
        Boolean(
          node?.textContent?.includes("25") && node.textContent.includes("€")
        )
      ).length
    ).toBeGreaterThan(0);
    fireEvent.change(screen.getByLabelText("Preferred name"), {
    ...
    expect(
      screen.getByRole("dialog", { name: "Review your order" })
    ).toBeInTheDocument();
    expect(
      document.querySelector(".order-preview-backdrop")
    ).toBeInTheDocument();
    ...
    await act(async () => {
      mutationOptions.onSuccess?.({
        invoiceUrl: "https://checkout.example/invoice/1",
      });
    });
  ...
  it("emails the generated receipt from the checkout success state and surfaces delivery errors", async () => {
    ...
    await act(async () => {
      mutationOptions.onSuccess?.({
        invoiceUrl: "https://checkout.example/invoice/2",
      });
    });
    ...
    expect(emailMutationState.mutate).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    await act(async () => {
      emailMutationOptions.onSuccess?.();
    });
    expect(screen.getByRole("status")).toHaveTextContent(
      "Receipt sent by email."
    );
    expect(
      screen.getByRole("button", { name: "Email this receipt" })
    ).toBeDisabled();
    expect(
      screen.getByText(/personal reflection and spiritual exploration/)
    ).toBeInTheDocument();
    await act(async () => {
      emailMutationOptions.onError?.();
    });
    expect(screen.getByRole("alert")).toHaveTextContent(
      "Could not send the receipt. Please try again."
    );
  });
});
