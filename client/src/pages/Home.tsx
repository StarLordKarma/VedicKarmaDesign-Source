...
const attachmentCopy: Record<Locale, { label: string; hint: string; remove: string; invalidType: string; tooLarge: string; selected: string }> = { en: { label: "Optional attachment", hint: "Add a PDF, image, or short note (up to 5 MB). It will be attached to your request.", remove: "Remove file", invalidType: "Please choose a PDF, JPG, PNG, WEBP, or TXT file.", tooLarge: "The attachment must be 5 MB or smaller.", selected: "Selected" }, ru: { label: "Дополнительный файл", hint: "Прикрепите PDF, изображение или короткую заметку (до 5 МБ). Файл будет добавлен к заявке.", remove: "Удалить файл", invalidType: "Выберите файл PDF, JPG, PNG, WEBP или TXT.", tooLarge: "Размер файла не должен превышать 5 МБ.", selected: "Выбрано" }, de: { label: "Optionaler Anhang", hint: "Fügen Sie eine PDF-, Bild- oder kurze Textdatei hinzu (bis 5 MB). Sie wird an Ihre Anfrage angehängt.", remove: "Datei entfernen", invalidType: "Bitte wählen Sie eine PDF-, JPG-, PNG-, WEBP- oder TXT-Datei.", tooLarge: "Der Anhang darf höchstens 5 MB groß sein.", selected: "Ausgewählt" }, es: { label: "Archivo opcional", hint: "Añade un PDF, imagen o nota breve (hasta 5 MB). Se adjuntará a tu solicitud.", remove: "Eliminar archivo", invalidType: "Elige un archivo PDF, JPG, PNG, WEBP o TXT.", tooLarge: "El archivo no puede superar los 5 MB.", selected: "Seleccionado" } };
...
export default function Home() {
  ...
  const [attachment, setAttachment] = useState<{ fileName: string; contentType: "application/pdf" | "image/jpeg" | "image/png" | "image/webp" | "text/plain"; size: number; contentBase64: string } | null>(null);
  ...
  async function handleAttachmentChange(event: React.ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; const allowed = ["application/pdf", "image/jpeg", "image/png", "image/webp", "text/plain"] as const; if (!allowed.includes(file.type as (typeof allowed)[number])) { setAttachment(null); setFormError(attachmentCopy[locale].invalidType); event.target.value = ""; return; } if (file.size > 5 * 1024 * 1024) { setAttachment(null); setFormError(attachmentCopy[locale].tooLarge); event.target.value = ""; return; } try { const contentBase64 = await fileToBase64(file); setAttachment({ fileName: file.name, contentType: file.type as (typeof allowed)[number], size: file.size, contentBase64 }); setFormError(""); } catch { setAttachment(null); setFormError(attachmentCopy[locale].invalidType); } }
