...
const draftCopy: Record<Locale, { restored: string; saved: string }> = { en: { restored: "Draft restored from this device", saved: "Draft saved locally" }, ru: { restored: "Черновик восстановлен с этого устройства", saved: "Черновик сохранён локально" }, de: { restored: "Entwurf von diesem Gerät wiederhergestellt", saved: "Entwurf lokal gespeichert" }, es: { restored: "Borrador restaurado en este dispositivo", saved: "Borrador guardado localmente" } };
...
export default function Home() {
  ...
  function saveBookingDraft(form: HTMLFormElement) { const value = (name: string) => String((form.elements.namedItem(name) as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement | null)?.value ?? ""); const draft: BookingDraft = { savedAt: Date.now(), name: value("name"), email: value("email"), birthDate: value("birthDate"), birthTime: value("birthTime"), birthCity: value("birthCity"), birthCountry: value("birthCountry"), language: value("language") || "English", interest: value("interest"), addon, currency, promoCode }; try { window.localStorage.setItem(BOOKING_DRAFT_KEY, JSON.stringify(draft)); setDraftStatus(draftCopy[locale].saved); } catch { /* Storage may be disabled; the form remains usable. */ } }
