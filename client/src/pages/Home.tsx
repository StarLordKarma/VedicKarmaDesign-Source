...
export default function Home() {
  ...
  const pricingBreakdownPdf = trpc.pricing.breakdownPdf.useMutation({ onSuccess: (result) => { const absoluteUrl = new URL(result.url, window.location.origin).toString(); setReceiptUrl(absoluteUrl); setLinkCopied(false); downloadBase64File(result.contentBase64, result.filename, "application/pdf"); }, onError: (error) => setFormError(getCheckoutErrorMessage(error.message)) });
