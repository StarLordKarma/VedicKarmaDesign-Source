...
function downloadBase64File(contentBase64: string, filename: string, contentType: string) { const bytes = Uint8Array.from(window.atob(contentBase64), (character) => character.charCodeAt(0)); const url = window.URL.createObjectURL(new Blob([bytes], { type: contentType })); const link = document.createElement("a"); link.href = url; link.download = filename; link.click(); window.setTimeout(() => window.URL.revokeObjectURL(url), 0); }
...
export default function Home() {
  ...
  const pricingBreakdownPdf = trpc.pricing.breakdownPdf.useMutation({ onSuccess: (result) => downloadBase64File(result.contentBase64, result.filename, "application/pdf"), onError: (error) => setFormError(getCheckoutErrorMessage(error.message)) });
  ...
  function downloadBreakdownPdf() { setFormError(""); pricingBreakdownPdf.mutate({ currency, locale, addon, labels: { title: copy.priceBreakdown, currency: currencyLabel, basic: copy.priceBasic, addon: copy.priceAddon, addonNotSelected: copy.priceAddonNotSelected, total: copy.priceTotal, generated: copy.generatedAt } }); }
