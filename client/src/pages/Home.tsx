...
export default function Home() {
  ...
  const emailReceipt = trpc.pricing.emailBreakdownPdf.useMutation({ onSuccess: () => { setReceiptEmailSent(true); setFormError(""); }, onError: () => setFormError(copy.receiptEmailFailed) });
  ...
  function receiptLabels() { return { title: copy.priceBreakdown, currency: currencyLabel, basic: copy.priceBasic, addon: copy.priceAddon, addonNotSelected: copy.priceAddonNotSelected, total: copy.priceTotal, generated: copy.generatedAt }; }
  function downloadBreakdownPdf() { setFormError(""); pricingBreakdownPdf.mutate({ currency, locale, addon, labels: receiptLabels() }); }
  function sendReceiptByEmail(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setFormError(""); emailReceipt.mutate({ email: receiptEmail.trim(), currency, locale, addon, language: ({ en: "English", ru: "Русский", de: "Deutsch", es: "Español" } as const)[locale], labels: receiptLabels() }); }
