...
export default function Home() {
  ...
  const [locale, setLocale] = useState<Locale>(() => new URLSearchParams(window.location.search).get("lang") === "ru" ? "ru" : "en");
  const copy = COPY[locale];
  const serviceHighlights = locale === "en" ? ["Multi-page PDF reading", "Clear, structured interpretation", "English, Russian, German & Spanish", "One round of clarification questions"] : ["Многостраничный PDF", "Понятная структурированная интерпретация", "Английский, русский, немецкий и испанский", "Один раунд уточняющих вопросов"];
      ...
      <header className="sticky top-0 z-50 border-b border-[#28231f]/10 bg-[#f8f5f0]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium text-[#635a52] md:flex">
            ...
            <button type="button" onClick={() => setLocale(locale === "en" ? "ru" : "en")} className="rounded-full border border-[#28231f]/15 px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em]">{locale === "en" ? "RU" : "EN"}</button>
        ...
        {menuOpen && (
          <nav className="border-t border-[#28231f]/10 bg-[#f8f5f0] px-5 py-4 md:hidden">
            <div className="flex flex-col gap-4 text-sm font-medium">
              ...
              <button type="button" onClick={() => setLocale(locale === "en" ? "ru" : "en")} className="rounded-full border border-[#28231f]/15 px-4 py-2 text-left text-xs font-semibold uppercase tracking-[0.12em]">{copy.localeLabel}: {locale === "en" ? "RU" : "EN"}</button>
      ...
      <main id="top">
          ...
          <div className="mx-auto grid max-w-7xl gap-14 px-5 pb-20 pt-16 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8 lg:pb-28 lg:pt-24">
            <div>
              ...
              <h1 className="max-w-3xl font-serif text-5xl leading-[0.98] tracking-[-0.045em] text-[#28231f] sm:text-6xl lg:text-8xl">
                {locale === "en" ? <>A clearer map for your <em className="font-normal text-[#b55b39]">inner sky.</em></> : <>Более ясная карта вашего <em className="font-normal text-[#b55b39]">внутреннего неба.</em></>}
              </h1>
            ...
            <div className="relative mx-auto w-full max-w-[500px]">
              ...
              <div className="absolute -bottom-5 -left-5 grid h-24 w-24 place-items-center rounded-2xl border border-[#e1b046]/40 bg-[#fffaf4] text-center shadow-xl">
                <div><p className="font-serif text-2xl text-[#b55b39]">${READING_PRICES.basic}</p><p className="text-[10px] uppercase tracking-widest text-[#776d63]">{copy.basicLabel}</p></div>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:items-start">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <article className="rounded-3xl border border-[#d9a441]/35 bg-[#f4ead5] p-7 sm:col-span-2 sm:flex sm:items-center sm:justify-between sm:gap-8">
                <div><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[#d9a441]/20 text-[#9c7012]"><Gem size={20} /></div><span className="text-xs font-semibold uppercase tracking-[0.18em] text-[#9c7012]">{copy.optional}</span></div><h3 className="mt-4 font-serif text-2xl">{copy.addOn}</h3><p className="mt-2 max-w-xl text-sm leading-6 text-[#675944]">{copy.addOnBody}</p></div><div className="mt-6 shrink-0 font-serif text-3xl text-[#9c7012] sm:mt-0">+${READING_PRICES.numerologyAddon}</div>
              </article>
            </div>
          </div>
        </section>
        ...
        <section id="book" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-[0.75fr_1.25fr]">
            <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#b55b39]">{copy.bookSection}</p><h2 className="mt-4 font-serif text-4xl leading-tight tracking-[-0.03em] sm:text-5xl">{copy.bookTitle}</h2><p className="mt-5 max-w-md text-base leading-7 text-[#635a52]">{copy.bookBody}</p><div className="mt-8 rounded-2xl border border-[#28231f]/10 bg-[#fffaf4] p-5 text-sm leading-6 text-[#635a52]"><strong className="text-[#28231f]">{locale === "en" ? "A note on the practice." : "Важно знать."}</strong> {copy.note}</div></div>
            <div className="rounded-[2rem] bg-[#28231f] p-6 text-[#fffaf4] shadow-[0_24px_70px_rgba(40,35,31,0.18)] sm:p-9">
              {submitted ? <div className="flex min-h-[520px] flex-col items-center justify-center text-center"><div className="grid h-16 w-16 place-items-center rounded-full bg-[#d9a441] text-[#28231f]"><Check size={30} /></div><h3 className="mt-7 font-serif text-4xl">{copy.successTitle}</h3><p className="mt-4 max-w-md leading-7 text-[#e8d9c0]">{locale === "en" ? getCheckoutSuccessMessage(total) : `Ваша заявка сохранена. Перейдите к безопасной криптооплате на сумму $${total}.`}</p><a href={paymentUrl} target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-3 rounded-full bg-[#e1b046] px-5 py-3 text-sm font-bold text-[#28231f] hover:-translate-y-0.5">{copy.continueCheckout} <ArrowRight size={16} /></a><button className="mt-4 rounded-full border border-[#fffaf4]/30 px-5 py-3 text-sm font-semibold text-[#fffaf4] hover:bg-[#fffaf4]/10" onClick={() => { setSubmitted(false); setPaymentUrl(""); setRedirecting(false); }}>{copy.anotherRequest}</button></div> : <form onSubmit={handleSubmit} className="space-y-6"><div className="flex items-start justify-between gap-4 border-b border-[#fffaf4]/15 pb-6"><div><p className="text-xs uppercase tracking-[0.18em] text-[#e1b046]">{copy.requestReading}</p><h3 className="mt-2 font-serif text-3xl">{copy.birthDetails}</h3></div><div className="text-right"><p className="font-serif text-3xl">${total}</p><p className="text-xs text-[#cdbfae]">{copy.pdfReading}{addon ? " + add-on" : ""}</p></div></div><div className="grid gap-5 sm:grid-cols-2"><label className="text-sm text-[#e8d9c0]">{copy.name}<input required name="name" className="mt-2 w-full rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none placeholder:text-[#b9aa97] focus:border-[#e1b046]" placeholder={copy.placeholderName} /></label><label className="text-sm text-[#e8d9c0]">{copy.email}<input required type="email" name="email" className="mt-2 w-full rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none placeholder:text-[#b9aa97] focus:border-[#e1b046]" placeholder="you@example.com" /></label><label className="text-sm text-[#e8d9c0]">{copy.date}<input required type="date" name="birthDate" className="mt-2 w-full rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none focus:border-[#e1b046]" /></label><label className="text-sm text-[#e8d9c0]">{copy.time}<input required type="time" name="birthTime" className="mt-2 w-full rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none focus:border-[#e1b046]" /></label><label className="text-sm text-[#e8d9c0]">{copy.city}<input required name="birthCity" className="mt-2 w-full rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none placeholder:text-[#b9aa97] focus:border-[#e1b046]" placeholder={copy.placeholderCity} /></label><label className="text-sm text-[#e8d9c0]">{copy.country}<input required name="birthCountry" className="mt-2 w-full rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none placeholder:text-[#b9aa97] focus:border-[#e1b046]" placeholder={copy.placeholderCountry} /></label></div><label className="block text-sm text-[#e8d9c0]">{copy.language}<select name="language" className="mt-2 w-full rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none focus:border-[#e1b046]"><option className="text-[#28231f]">English</option><option className="text-[#28231f]">Русский</option><option className="text-[#28231f]">Deutsch</option><option className="text-[#28231f]">Español</option></select></label><label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-[#fffaf4]/15 bg-[#fffaf4]/5 p-4"><input type="checkbox" checked={addon} onChange={(event) => setAddon(event.target.checked)} className="mt-1 h-4 w-4 accent-[#d9a441]" /><span><span className="block text-sm font-semibold text-[#fffaf4]">{copy.addOn} <span className="text-[#e1b046]">+${READING_PRICES.numerologyAddon}</span></span><span className="mt-1 block text-xs leading-5 text-[#cdbfae]">{copy.addOnBody}</span></span></label><label className="block text-sm text-[#e8d9c0]">{copy.interest}<textarea name="interest" rows={3} className="mt-2 w-full resize-none rounded-xl border border-[#fffaf4]/15 bg-[#fffaf4]/8 px-4 py-3 text-[#fffaf4] outline-none placeholder:text-[#b9aa97] focus:border-[#e1b046]" placeholder={copy.optionalInterest} /></label>{formError && <p role="alert" className="rounded-xl border border-red-300/40 bg-red-900/20 px-4 py-3 text-sm text-[#ffd8cf]">{formError}</p>}<button type="submit" disabled={submitBooking.isPending} className="group flex w-full items-center justify-center gap-3 rounded-full bg-[#e1b046] px-6 py-4 text-sm font-bold text-[#28231f] transition-transform hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-70">{submitBooking.isPending ? <><Loader2 size={17} className="animate-spin" /> {locale === "en" ? getCheckoutButtonLabel(true) : "Создание защищённой оплаты…"}</> : <>{copy.request} <ArrowRight size={17} className="transition-transform group-hover:translate-x-1" /></>}</button><p className="text-center text-xs leading-5 text-[#b9aa97]">{copy.privacy}</p></form>}
            </div>
          </div>
        </section>
